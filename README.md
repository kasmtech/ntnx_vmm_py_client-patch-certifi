...
## Configuration
The python client for Nutanix Virtual Machine Management APIs can be configured with the following parameters
...
```python
# Initialize the API
image_placement_policies_api_instance = ImagePlacementPoliciesApi(api_client=client) # client configured in previous step
extId = 'extId_example' # UUID.

# Get an image placement policy
try:
    api_response = image_placement_policies_api_instance.get_placement_policy_by_id(extId)
...
try:
    ...
    api_response = image_placement_policies_api_instance.update_placement_policy_by_id(body, extId, if_match=etag_value) # Use the extracted etag value
except ApiException as e:
```
...
```python
# Initialize the API
esxi_stats_api_instance = EsxiStatsApi(api_client=client) # client configured in previous step
extId = 'extId_example' # UUID.

# List VM stats for all VMs
try:
    api_response = esxi_stats_api_instance.list_vm_stats(
...
This library has a full set of [API Reference Documentation](https://developers.nutanix.com/sdk-reference?namespace=vmm&version=v4.1&language=python). This documentation is auto-generated, and the location may change.
